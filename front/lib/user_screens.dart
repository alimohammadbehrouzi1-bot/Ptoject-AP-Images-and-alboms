  ...
  void _loadMore() {
    ...
    setState(() {
      List<FileItem> source = _currentAlbum == null 
          ...
          : _userItems.where((i) => !i.isFolder && i.parentId == _currentAlbum!.id).toList();
      ...
      body: _foundUsername == null 
        ...
        : GridView.builder(
            ...
            itemBuilder: (context, i) {
              if (i >= _visibleItems.length) return const SizedBox(height: 50);
              final item = _visibleItems[i];
              return InkWell(
                onTap: () {
                  if (item.isFolder) {
                    setState(() { _currentAlbum = item; _refreshData(); });
                  } else {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => ImageDetailScreen(item: item, username: widget.currentUsername, isReadOnly: true)));
                  }
                },
                child: Column(
                  children: [
...
class _MyStuffsPageState extends State<MyStuffsPage> {
  ...
  List<FileItem> _getFilteredSource() {
    if (_searchQ.isNotEmpty) {
      ...
      return _allVaultItems.where((item) {
        if (q.startsWith('#')) return !item.isFolder && (item.tags?.any((t) => t.toLowerCase().contains(q.substring(1))) ?? false);
        if (_filters.isEmpty) return item.name.toLowerCase().contains(q) || (!item.isFolder && (item.caption?.toLowerCase().contains(q) ?? false));
        bool m = false;
        if (_filters.contains('Name') && item.name.toLowerCase().contains(q)) m = true;
        if (_filters.contains('Caption') && !item.isFolder && (item.caption?.toLowerCase().contains(q) ?? false)) m = true;
        if (_filters.contains('Tags') && !item.isFolder && (item.tags?.any((t) => t.toLowerCase().contains(q)) ?? false)) m = true;
        return m;
      }).toList();
    }
    return _currentAlbum == null 
        ? _allVaultItems.where((i) => i.isFolder || i.parentId == null).toList()
        : _allVaultItems.where((i) => !i.isFolder && i.parentId == _currentAlbum!.id).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: _isSelectionMode ? IconButton(icon: const Icon(Icons.close), onPressed: () => setState((){_selectedIds.clear(); _isSelectionMode=false;})) 
          : (_currentAlbum != null ? IconButton(icon: const Icon(Icons.arrow_back_ios_new, size: 20), onPressed: () => setState(() { _currentAlbum = null; _refresh(); })) : null),
        actions: _isSelectionMode ? [
          if (canMove) IconButton(icon: const Icon(Icons.drive_file_move_outlined, color: Colors.blue), onPressed: () => _showMoveDialog()),
      ...
      child: Row(
        children: [
          _btn(Icons.add_a_photo, "Upload", Colors.blue, () async {
            final res = await Navigator.push(context, MaterialPageRoute(builder: (_) => UploadImageScreen(albums: _allVaultItems.where((i)=>i.isFolder).toList())));
            if (res != null) { DataService().addImage(widget.username, res); _refresh(); }
          }),
          const SizedBox(width: 12),
          if (_currentAlbum == null) _btn(Icons.folder_open, "Album", Colors.indigo, () => _showCreateAlbum()),
        ],
      ),
    );
  }
  ...
  void _showCreateAlbum() {
    ...
    showDialog(context: context, builder: (context) => AlertDialog(title: const Text('New Album'), content: TextField(controller: c), actions: [ElevatedButton(onPressed: (){DataService().addAlbum(widget.username, c.text); _refresh(); Navigator.pop(context);}, child: const Text('Create'))]));
  }

  void _showMoveDialog() {
    showDialog(context: context, builder: (context) => AlertDialog(title: const Text('Move to...'), content: SizedBox(width: double.maxFinite, child: ListView(shrinkWrap: true, children: [ListTile(title: const Text('Root'), onTap: (){ DataService().movePhotos(_selectedIds.toList(), null); _refresh(); Navigator.pop(context); setState(()=>_isSelectionMode=false); }), ..._allVaultItems.where((i) => i.isFolder).map((f) => ListTile(title: Text(f.name), onTap: (){ DataService().movePhotos(_selectedIds.toList(), f.id); _refresh(); Navigator.pop(context); setState(()=>_isSelectionMode=false); }))]))));
  }

  void _showUserSheet() {
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(32))), builder: (context) => Container(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [CircleAvatar(radius: 40, child: Text(widget.username[0].toUpperCase())), const SizedBox(height: 16), Text(widget.username, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), const SizedBox(height: 32), ElevatedButton(onPressed: (){Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => ProfileDetailScreen(username: widget.username)));}, child: const Text('Settings')), const SizedBox(height: 12), OutlinedButton(onPressed: (){DataService().logout(); Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_)=>const LoginScreen()), (r)=>false);}, child: const Text('Log Out'))])));
  }
}
            ...
            ...widget.item.comments.map((c) => ListTile(
              ...
              trailing: Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(c.isLiked ? Icons.favorite : Icons.favorite_border, size: 16, color: c.isLiked ? Colors.red : Colors.grey), 
                    onPressed: () {
                      setState(() { 
                        c.isLiked = !c.isLiked; 
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(controller: captionC, decoration: const InputDecoration(labelText: 'Caption')),
                const SizedBox(height: 16),
                Row(
                  children: [
                    Expanded(child: TextField(controller: tagC, decoration: const InputDecoration(hintText: 'Add tag'))),
                    IconButton(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                DataService().updatePhoto(widget.item.id, captionC.text, tempTags);
                Navigator.pop(context);
              },
...
class _ProfileDetailScreenState extends State<ProfileDetailScreen> {
  ...
  void _showChangeName() {
    final c = TextEditingController();
    showDialog(context: context, builder: (context)=>AlertDialog(title: const Text('New Username'), content: TextField(controller: c), actions: [ElevatedButton(onPressed: (){ if(DataService().changeUsername(widget.username, c.text)){ Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>MainNavigation(username: c.text)), (r)=>false); } }, child: const Text('Change'))]));
  }
  ...
  void _showDeleteConfirm() {
    showDialog(context: context, builder: (context)=>AlertDialog(title: const Text('Delete Account?'), content: const Text('This action cannot be undone.'), actions: [TextButton(onPressed: ()=>Navigator.pop(context), child: const Text('Cancel')), ElevatedButton(onPressed: (){ DataService().deleteAccount(widget.username); Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>const LoginScreen()), (r)=>false); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red), child: const Text('Delete'))]));
  }
}
